    ...
    std::vector<cv::Mat> & tvecs)
{
    ...
    auto pattern_cols = yaml_read<int>(yaml, "pattern_cols");
    auto pattern_rows = yaml_read<int>(yaml, "pattern_rows");
    auto center_distance_mm = yaml_read<double>(yaml, "center_distance_mm");
    R_gimbal2imubody_data = yaml_read<std::vector<double>>(yaml, "R_gimbal2imubody");
    ...
    for (int i = 1; true; i++) {
        ...
        Eigen::Vector3d ypr = eulers(R_gimbal2world, 2, 1, 0) * 57.3;  // degree

        // 在图片上显示云台的欧拉角，用来检验R_gimbal2imubody是否正确
        auto drawing = img.clone();
        draw_text(drawing, fmt::format("yaw   {:.2f}", ypr[0]), {40, 40}, {0, 0, 255});
        draw_text(drawing, fmt::format("pitch {:.2f}", ypr[1]), {40, 80}, {0, 0, 255});
        draw_text(drawing, fmt::format("roll  {:.2f}", ypr[2]), {40, 120}, {0, 0, 255});

        // 识别标定板
        std::vector<cv::Point2f> centers_2d;
        auto success = cv::findChessboardCorners(img, pattern_size, centers_2d);  // 默认是方形网格图案
...
int main(int argc, char * argv[])
{
    ...
    Eigen::Vector3d camera_ypr = eulers(R_camera2ideal, 1, 0, 2) * 57.3;  // degree
    ...
    Eigen::Vector3d board_ypr = eulers(R_board2world_eigen, 2, 1, 0) * 57.3;  // degree

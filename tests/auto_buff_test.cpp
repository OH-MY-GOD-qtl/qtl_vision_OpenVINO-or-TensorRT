...
int main(int argc, char * argv[])
{
    ...
    auto start_index = cli.get<int>(1);
    auto end_index = cli.get<int>(2);
    auto input_path = cli.get<std::string>(3);
    ...
    Buff_Detector detector(config_path);
    BuffSolver solver(config_path);
    // SmallTarget target;
    BigTarget target;
    BuffAimer aimer(config_path);
    ...
    for (int frame_count = start_index; !exiter.exit(); frame_count++) {
        ...
        if (!target.is_unsolve()) {
            ...
            // 显示
            for (int i = 0; i < 4; i++) draw_point(img, p.target().points[i]);
            draw_point(img, p.target().center, {0, 0, 255}, 3);
            draw_point(img, p.r_center, {0, 0, 255}, 3);

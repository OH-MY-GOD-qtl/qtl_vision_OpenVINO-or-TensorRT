...
const std::string keys =
    ...
    "{@delta-angle   | 8                   | yaw轴delta角}"
    "{@circle        | 0.2                 | delta_angle的切片数}"
    "{@signal-mode   | triangle_wave       | 发送信号的模式}"
    "{@axis          | yaw                 | 发送信号的轴}";
...
int main(int argc, char * argv[])
{
    ...
    auto delta_angle = cli.get<double>(1);
    auto circle = cli.get<double>(2);
    auto signal_mode = cli.get<std::string>(3);
    auto axis = cli.get<std::string>(4);
    ...
    while (!exiter.exit()) {
        ...
        Eigen::Vector3d ypr = eulers(q, 2, 1, 0);
        ...
        if (signal_mode == "triangle_wave") {
            ...
            command = {1, 0, limit_rad(cmd_angle / 57.3), 0};
            count++;

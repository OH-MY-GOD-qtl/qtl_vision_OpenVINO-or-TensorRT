...
{
    logger()->info("[Cboard] Waiting for q...");
    queue_.pop(data_ahead_);
    queue_.pop(data_behind_);
    logger()->info("[Cboard] Opened.");
}
...
void CBoard::callback(const can_frame & frame)
{
    ...
    if (frame.can_id == quaternion_canid_) {
        ...
        if (bullet_speed > 0 && delta_time(now, last_log_time) >= 1.0) {
            logger()->info(
...
std::string CBoard::read_yaml(const std::string & config_path)
{
    ...
    quaternion_canid_ = yaml_read<int>(yaml, "quaternion_canid");
    bullet_speed_canid_ = yaml_read<int>(yaml, "bullet_speed_canid");
    send_canid_ = yaml_read<int>(yaml, "send_canid");

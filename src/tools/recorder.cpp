...
void Recorder::save_to_file()
{
    while (!stop_thread_) {
        ...
        if (frame.img.empty()) {
            logger()->debug("Recorder received empty img. Skip this frame.");
            continue;
        }
        // 写入视频文件
        ...
        auto since_begin = delta_time(frame.timestamp, start_time_);
    ...
    const std::chrono::steady_clock::time_point & timestamp)
{
    ...
    auto since_last = delta_time(timestamp, last_time_);

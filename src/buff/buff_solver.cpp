#include "buff/buff_solver.hpp"
#include "tools/yaml.hpp"

cv::Matx33f BuffSolver::rotation_matrix(double angle) const
{
    return cv::Matx33f(
        1, 0, 0, 0, std::cos(angle), -std::sin(angle), 0, std::sin(angle), std::cos(angle));
}

void BuffSolver::compute_rotated_points(std::vector<std::vector<cv::Point3f>> & object_points)
{
...
BuffSolver::BuffSolver(const std::string & config_path) : R_gimbal2world_(Eigen::Matrix3d::Identity())
{
    auto yaml = yaml_load(config_path);

    auto R_gimbal2imubody_data = yaml_read<std::vector<double>>(yaml, "R_gimbal2imubody");
    auto R_camera2gimbal_data = yaml_read<std::vector<double>>(yaml, "R_camera2gimbal");
    auto t_camera2gimbal_data = yaml_read<std::vector<double>>(yaml, "t_camera2gimbal");
    R_gimbal2imubody_ = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(R_gimbal2imubody_data.data());
    R_camera2gimbal_ = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(R_camera2gimbal_data.data());
    t_camera2gimbal_ = Eigen::Matrix<double, 3, 1>(t_camera2gimbal_data.data());

    auto camera_matrix_data = yaml_read<std::vector<double>>(yaml, "camera_matrix");
    auto distort_coeffs_data = yaml_read<std::vector<double>>(yaml, "distort_coeffs");
...
Eigen::Matrix3d BuffSolver::R_gimbal2world() const { return R_gimbal2world_; }

void BuffSolver::set_R_gimbal2world(const Eigen::Quaterniond & q)
{
    Eigen::Matrix3d R_imubody2imuabs = q.toRotationMatrix();
    R_gimbal2world_ = R_gimbal2imubody_.transpose() * R_imubody2imuabs * R_gimbal2imubody_;
}

void BuffSolver::solve(std::optional<PowerRune> & ps) const
{
    ...
    p.ypd_in_world = xyz2ypd(p.xyz_in_world);

    p.blade_xyz_in_world = R_gimbal2world_ * blade_xyz_in_gimbal;
    p.blade_ypd_in_world = xyz2ypd(p.blade_xyz_in_world);

    p.ypr_in_world = eulers(R_buff2world, 2, 1, 0);
}

// 调试用
cv::Point2f BuffSolver::point_buff2pixel(cv::Point3f x)
{
    // buff坐标系(单位:m)到像素坐标系
...
std::vector<cv::Point2f> BuffSolver::reproject_buff(
    const Eigen::Vector3d & xyz_in_world, double yaw, double row) const
{
    auto R_buff2world = ::rotation_matrix(Eigen::Vector3d(yaw, 0.0, row));
    // clang-format on
